...
The other, the sealing key, is one whose address (sometimes referred to as 'value' or 'cursor') does not represent a memory location, but instead represents a value in an abstract space of types and has the permit-seal permission.
The address of the sealing key is placed into the otype field of the pointer-like capability, giving a *sealed* capability.
...
CHERI (hardware) capabilities are unforgeable (delegable) tokens of authority that authorise access to memory.
CHERIoT software capabilities are unforgeable (delegable) tokens of authority that authorise some other compartment to perform some action on your behalf.
...
Sealed capabilities are passed out and back as opaque values and can be unsealed without any global state beyond the sealing key.
...
While operating on your TLS session, that compartment does not hold any capabilities to other TLS sessions.
...
If you compromise this compartment, you can tamper with a queue that you hold an endpoint handle for, but not any other queue.
...
When you jump to a sentry, it is implicitly unsealed.
...
We have sentry variants that explicitly enable or disable interrupts when you jump to them.
